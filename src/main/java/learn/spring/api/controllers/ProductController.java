...
public class ProductController {
    ...
    public ResponseEntity<List<ProductModel>> getALlProducts() {
        return ResponseEntity.status(HttpStatus.OK).body(productRepository.findAll()); //retorna o estado HTTP, junto com todos os produtos cadastrados
    }
    ...
    public ResponseEntity<Object> getOneProduct(@PathVariable(value="id") UUID id){
        ...
        if(product0.isEmpty()){
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Product not found"); //verifica se recebeu alguma coisa, caso não, retorna que não achou.
        }
